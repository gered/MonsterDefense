...
void TextureAnimator::AddTileSequence(const stl::string &name, TextureAtlas *atlas, uint32_t tileToBeAnimated, uint32_t start, uint32_t stop, float delay, BOOL loop)
{
	...
	sequence.originalAnimatingTile = Image::CreateFrom(textureImage, originalTile.dimensions.left, originalTile.dimensions.top, originalTile.dimensions.right - 1, originalTile.dimensions.bottom - 1);

	// copy each frame ("start" to "stop") from the source texture image
	for (uint32_t i = 0; i < sequence.GetNumFrames(); ++i)
	{
		const TextureAtlasTile &tile = atlas->GetTile(i + sequence.start);
		sequence.frames[i] = Image::CreateFrom(textureImage, tile.dimensions.left, tile.dimensions.top, tile.dimensions.right - 1, tile.dimensions.bottom - 1);
	}
...
void TextureAnimator::UpdateTextureWithCurrentTileFrame(TextureAtlasAnimationSequence &sequence)
{
	...
	m_graphicsDevice->UpdateTextureFromImage(sequence.atlas->GetTexture(), frameImage, tile.dimensions.left, tile.dimensions.top);
}
...
void TextureAnimator::RestoreTextureWithOriginalTile(TextureAtlasAnimationSequence &sequence)
{
	STACK_TRACE;
	const TextureAtlasTile &tile = sequence.atlas->GetTile(sequence.animatingIndex);
	m_graphicsDevice->UpdateTextureFromImage(sequence.atlas->GetTexture(), sequence.originalAnimatingTile, tile.dimensions.left, tile.dimensions.top);
}
