...
const float TEXT_START_SCALE = 2.0f;
const float TEXT_MAX_SCALE = 6.0f;

const float GWEN_ANIMATE_TIME = 0.5f;

MissionFailureState::MissionFailureState(GwenGameState *gameState)
...
MissionFailureState::~MissionFailureState()
{
	STACK_TRACE;
}

void MissionFailureState::OnPush()
{
	STACK_TRACE;
...
void MissionFailureState::SetupControls()
{
	STACK_TRACE;
	...
	btnRetry->onPress.Add(this, &MissionFailureState::OnRetry);
	
	btnQuit->SetSize(BUTTON_WIDTH, BUTTON_HEIGHT);
	btnQuit->SetText("Quit");
	btnQuit->onPress.Add(this, &MissionFailureState::OnQuit);
...
void MissionFailureState::OnPop()
{
	STACK_TRACE;
}

void MissionFailureState::OnRender(RenderContext *renderContext)
{
	STACK_TRACE;
...
void MissionFailureState::OnResize()
{
	STACK_TRACE;
...
void MissionFailureState::OnUpdate(float delta)
{
	STACK_TRACE;
...
BOOL MissionFailureState::OnTransition(float delta, BOOL isTransitioningOut, BOOL started)
{
	STACK_TRACE;
...
void MissionFailureState::OnRetry(Gwen::Controls::Base *sender)
{
	STACK_TRACE;
...
void MissionFailureState::OnQuit(Gwen::Controls::Base *sender)
{
	STACK_TRACE;
	QuitGameEvent event;
