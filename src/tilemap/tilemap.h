...
public:
	...
	Tile* GetTileAt(PositionComponent *position) const;
	void GetCoordsAt(PositionComponent *position, uint32_t &x, uint32_t &y, uint32_t &z) const;
	TileChunk* GetChunkAt(PositionComponent *position) const;
	Tile* GetTileUnder(PositionComponent *position) const;
	BOOL GetCoordsUnder(PositionComponent *position, uint32_t &x, uint32_t &y, uint32_t &z) const;
	Tile* GetTileAtFeetOf(PositionComponent *position, BoundingSphereComponent *bounds) const;
	BOOL GetCoordsAtFeetOf(PositionComponent *position, BoundingSphereComponent *bounds, uint32_t &x, uint32_t &y, uint32_t &z) const;
	Tile* GetTileAtCollisionWith(PhysicsComponent *physics) const;
	BOOL GetCoordsAtCollisionWith(PhysicsComponent *physics, uint32_t &x, uint32_t &y, uint32_t &z) const;
...
inline Tile* TileMap::GetTileAt(PositionComponent *position) const
{
	return Get(
		(uint32_t)position->position.x, 
		(uint32_t)position->position.y, 
		(uint32_t)position->position.z
		);
}

inline void TileMap::GetCoordsAt(PositionComponent *position, uint32_t &x, uint32_t &y, uint32_t &z) const
{
	x = (uint32_t)position->position.x;
	y = (uint32_t)position->position.y;
	z = (uint32_t)position->position.z;
}

inline TileChunk* TileMap::GetChunkAt(PositionComponent *position) const
{
	return GetChunkContaining(
		(uint32_t)position->position.x, 
		(uint32_t)position->position.y, 
		(uint32_t)position->position.z
		);
