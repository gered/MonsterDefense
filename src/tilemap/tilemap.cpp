...
#include "../game/entities/components/boundingspherecomponent.h"
#include "../game/entities/components/physicscomponent.h"
...
Tile* TileMap::GetTileUnder(PositionComponent *position) const
{
	STACK_TRACE;
	uint32_t x = 0;
	uint32_t y = 0;
	uint32_t z = 0;
	if (GetCoordsUnder(position, x, y, z))
		return Get(x, y, z);
	else
		return NULL;
}

BOOL TileMap::GetCoordsUnder(PositionComponent *position, uint32_t &x, uint32_t &y, uint32_t &z) const
{
	STACK_TRACE;
	Ray downward(position->position, DOWN);
	return CheckForCollision(downward, x, y, z);
}

Tile* TileMap::GetTileAtFeetOf(PositionComponent *position, BoundingSphereComponent *bounds) const
{
	STACK_TRACE;
	uint32_t x = 0;
	uint32_t y = 0;
	uint32_t z = 0;
	if (GetCoordsAtFeetOf(position, bounds, x, y, z))
		return Get(x, y, z);
	else
		return NULL;
}

BOOL TileMap::GetCoordsAtFeetOf(PositionComponent *position, BoundingSphereComponent *bounds, uint32_t &x, uint32_t &y, uint32_t &z) const
{
	STACK_TRACE;
	if (bounds != NULL)
	{
		float radius = bounds->bounds.radius;
		x = (uint32_t)position->position.x;
		y = (uint32_t)(position->position.y - radius);
		z = (uint32_t)position->position.z;
		return TRUE;
	}
	else
		return FALSE;
}
Tile* TileMap::GetTileAtCollisionWith(PhysicsComponent *physics) const
{
	STACK_TRACE;
	uint32_t x = 0;
	uint32_t y = 0;
	uint32_t z = 0;
	if (GetCoordsAtCollisionWith(physics, x, y, z))
		return Get(x, y, z);
	else
		return NULL;
}

BOOL TileMap::GetCoordsAtCollisionWith(PhysicsComponent *physics, uint32_t &x, uint32_t &y, uint32_t &z) const
{
	STACK_TRACE;
	if (physics->collisionPacket.foundCollision || physics->isSliding)
	{
		x = (uint32_t)physics->nearestIntersection.x;
		y = (uint32_t)physics->nearestIntersection.y;
		z = (uint32_t)physics->nearestIntersection.z;
		return TRUE;
	}
	else
		return FALSE;
