...
void TileMapRenderer::Render(const TileMap *tileMap, Shader *shader)
{
	...
	if (shader == NULL)
	{
		...
		m_graphicsDevice->GetSimpleColorTextureShader()->SetModelViewMatrix(m_graphicsDevice->GetModelViewMatrix());
		m_graphicsDevice->GetSimpleColorTextureShader()->SetProjectionMatrix(m_graphicsDevice->GetProjectionMatrix());
	}
	else
	{
	...
	for (uint32_t y = 0; y < tileMap->GetHeightInChunks(); ++y)
	{
		for (uint32_t z = 0; z < tileMap->GetDepthInChunks(); ++z)
		{
			for (uint32_t x = 0; x < tileMap->GetWidthInChunks(); ++x)
			{
				...
				if (m_graphicsDevice->GetCamera()->GetFrustum()->Test(chunk->GetBounds()))
				{
...
void TileMapRenderer::RenderAlpha(const TileMap *tileMap, Shader *shader)
{
	...
	for (uint32_t y = 0; y < tileMap->GetHeightInChunks(); ++y)
	{
		for (uint32_t z = 0; z < tileMap->GetDepthInChunks(); ++z)
		{
			...
			{
				TileChunk *chunk = tileMap->GetChunk(x, y, z);
				if (chunk->IsAlphaEnabled() && m_graphicsDevice->GetCamera()->GetFrustum()->Test(chunk->GetBounds()))
				{
