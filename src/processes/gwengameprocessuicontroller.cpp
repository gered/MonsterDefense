...
Gwen::Controls::Canvas* GwenGameProcessUIController::InitializeGwen(const stl::string &skinFilename, const stl::string &fontFilename, uint8_t fontSize)
{
	...
	m_renderer = new Gwen::Renderer::SpriteBatchRenderer(GetGameProcess()->GetGameApp()->GetContentManager());
...
void GwenGameProcessUIController::ResizeAndScaleCanvas()
{
	...
	if (m_canvas != NULL)
	{
		...
		uint16_t scaledViewportWidth = (float)GetGameProcess()->GetGameApp()->GetGraphicsDevice()->GetViewportWidth() / m_canvas->Scale();
		uint16_t scaledViewportHeight = (float)GetGameProcess()->GetGameApp()->GetGraphicsDevice()->GetViewportHeight() / m_canvas->Scale();
