...
Camera::Camera(GraphicsDevice *graphicsDevice)
{
	STACK_TRACE;
	m_graphicsDevice = graphicsDevice;
	m_frustum = new Frustum(m_graphicsDevice);
	...
	CalculateProjection(m_graphicsDevice->GetViewportLeft(), m_graphicsDevice->GetViewportTop(), m_graphicsDevice->GetViewportRight(), m_graphicsDevice->GetViewportBottom());
}
...
void Camera::CalculateProjection(uint16_t left, uint16_t top, uint16_t right, uint16_t bottom)
{
	STACK_TRACE;
...
{
	STACK_TRACE;
	m_graphicsDevice->SetModelViewMatrix(m_lookAt);
	m_frustum->Calculate();
}
...
{
	STACK_TRACE;
	CalculateProjection(size.left, size.top, size.right, size.bottom);
	m_graphicsDevice->SetProjectionMatrix(m_projection);
}

void Camera::OnResize(uint16_t left, uint16_t top, uint16_t right, uint16_t bottom)
{
	STACK_TRACE;
...
{
	STACK_TRACE;
	float nx = 2.0f * ((float)(screenX - (m_graphicsDevice->GetViewportWidth() / 2))) / ((float)m_graphicsDevice->GetViewportWidth());
	float ny = 2.0f * -((float)(screenY - (m_graphicsDevice->GetViewportHeight() / 2))) / ((float)m_graphicsDevice->GetViewportHeight());
...
Point2 Camera::Project(const Vector3 &objectPosition, const Matrix4x4 &modelview, const Matrix4x4 &projection) const
{
	...
	Point2 out;
	out.x = (int32_t)(((transformedX * 0.5f) + 0.5f) * (float)m_graphicsDevice->GetViewportWidth() + (float)m_graphicsDevice->GetViewportLeft());
	out.y = (int32_t)(((transformedY * 0.5f) + 0.5f) * (float)m_graphicsDevice->GetViewportHeight() + (float)m_graphicsDevice->GetViewportTop());
	...
	out.y = (int32_t)m_graphicsDevice->GetViewportHeight() - out.y;
