...
void SpriteFont::Load(Texture *texture, TextureAtlas *glyphs, const stl::string &file, uint8_t size)
{
	STACK_TRACE;
	m_texture = texture;
	m_glyphs = glyphs;

	// if we have a font filename string already, assume the one provided in the function call is the same
	if (m_fontFile.length() == 0)
	{
		m_fontFile = file;
		m_size = size;
	}
