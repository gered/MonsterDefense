...
void SpriteBatch::InternalBegin(const RenderState *renderState, const BlendState *blendState, SpriteShader *shader)
{
	...
	m_previousProjection = m_graphicsDevice->GetProjectionMatrix();
	m_previousModelview = m_graphicsDevice->GetModelViewMatrix();
...
{
	STACK_TRACE;
	Point2 screenCoordinates = m_graphicsDevice->GetCamera()->Project(worldPosition, m_previousModelview, m_previousProjection);
...
void SpriteBatch::End()
{
	...
	m_shader->SetProjectionMatrix(m_graphicsDevice->GetOrthographicProjectionMatrix());
	RenderQueue();
...
void SpriteBatch::RenderQueueRange(const SpriteBatchEntity *firstEntity, const SpriteBatchEntity *lastEntity)
{
	...
	if (lastEntity->texture != NULL)
	{
		...
		m_shader->SetTextureHasAlphaOnly(lastEntity->texture->GetFormat() == IMAGE_FORMAT_ALPHA);
	}
	else
...
{
	return m_graphicsDevice->GetViewportHeight() - y - sourceHeight;
}

inline float SpriteBatch::FixYCoord(int16_t y, float sourceHeight) const
{
	return (float)m_graphicsDevice->GetViewportHeight() - (float)y - sourceHeight;
}
...
void SpriteBatch::SetClipRegion(const Rect &rect)
{
	...
	int32_t fixedTop = ((int32_t)m_graphicsDevice->GetViewportHeight() - rect.top - rect.GetHeight());
...
void SpriteBatch::SetClipRegion(int16_t left, int16_t top, int16_t right, int16_t bottom)
{
	...
	int16_t fixedTop = (m_graphicsDevice->GetViewportHeight() - top - height);
