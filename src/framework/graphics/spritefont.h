...
public:
	 ...
	 * @param file the filename of the original font that this is based on
	 * @param size the size that the glyphs were rendered at
	 */
	void Load(Texture *texture, TextureAtlas *glyphs, const stl::string &file, uint8_t size);
	...
	/**
	 * @return the filename of the original font this is based on
	 */
	const stl::string& GetFontFile() const                 { return m_fontFile; }
