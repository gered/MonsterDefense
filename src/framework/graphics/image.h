...
public:
	 ...
	 * @return an image object or NULL on failure
	 */
	static Image* CreateFrom(File *file);
	 ...
	 * @return an image object or NULL on failure
	 */
	static Image* CreateFrom(const Image *source);
	 ...
	 * @return an image object or NULL on failure
	 */
	static Image* CreateFrom(const Image *source, uint16_t left, uint16_t top, uint16_t right, uint16_t bottom);
	 ...
	 * @return an image object or NULL on failure
	 */
	static Image* Create(uint16_t width, uint16_t height, IMAGE_FORMAT format);
...
private:
	Image(uint8_t *pixels, BOOL didWeAllocateMem, uint16_t width, uint16_t height, uint8_t bpp, IMAGE_FORMAT format);

	BOOL m_didWeAllocateMem;
	uint8_t *m_pixels;
	uint32_t m_pitch;
	uint16_t m_width;
	uint16_t m_height;
