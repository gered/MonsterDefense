...
StaticMeshFile::StaticMeshFile(File *file, const Matrix4x4 &transform)
	: MeshFile(file)
{
	STACK_TRACE;
	m_vertices = NULL;
...
void StaticMeshFile::Load(const Matrix4x4 &transform)
{
	STACK_TRACE;
	...
	for (uint32_t i = 0; i < m_numVertices; ++i)
	{
		...
		m_vertices[i] = m_vertices[i] * transform;
	}
