...
KeyframeMeshFile::KeyframeMeshFile(File *file, const Matrix4x4 &transform)
	: MeshFile(file)
{
	STACK_TRACE;
	m_numFrames = 0;
...
void KeyframeMeshFile::Load(const Matrix4x4 &transform)
{
	STACK_TRACE;
	...
	for (uint32_t i = 0; i < m_numFrames; ++i)
	{
		...
		for (uint32_t j = 0; j < numVertices; ++j)
		{
			...
			vertices[j] = vertices[j] * transform;
		}
	...
	if (animationsDesc != NULL)
	{
		...
		for (uint32_t i = 0; i < numAnimations; ++i)
		{
			...
			// sequence names are variable length null terminated strings
			char c;
			do
			{
				c = file->ReadChar();
