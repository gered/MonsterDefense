...
Gwen::Controls::Canvas* GwenGameStateUIController::InitializeGwen(const stl::string &skinFilename, const stl::string &fontFilename, uint8_t fontSize)
{
	...
	m_renderer = new Gwen::Renderer::SpriteBatchRenderer(GetGameState()->GetGameApp()->GetContentManager());
...
void GwenGameStateUIController::ResizeAndScaleCanvas()
{
	...
	if (m_canvas != NULL)
	{
		...
		uint16_t scaledViewportWidth = (float)GetGameState()->GetGameApp()->GetGraphicsDevice()->GetViewportWidth() / m_canvas->Scale();
		uint16_t scaledViewportHeight = (float)GetGameState()->GetGameApp()->GetGraphicsDevice()->GetViewportHeight() / m_canvas->Scale();
